...
Calculator ::Calculator() : wxFrame(nullptr, wxID_ANY, "Calculator", wxPoint(30, 30), wxSize(800, 600)) { // create Window
	...
	// Used to create side by side menu for debug
	...
	// Create vectors of buttons. Buttons will be added in this order
	vector<string> characters = {"C" , "( )" , "%" , "/" , "7" , "8" ,"9" , "*", "4", "5","6" ,"-", "1", "2", "3",
		"+", "~", "0", "." , "=", "^" };
...
vector<string> Calculator::infixToPostfix(vector<string> &s)
{
	...
	for (int i = 0; i < l; i++)
	{
		...
		if (all_of(s[i].begin(), s[i].end(), ::isdigit) != 0 && s[i] != '(' && s[i] != ')') {
